...
st.success(f"✅ Data siap! {df.shape[0]} responden, {df.shape[1]} soal")

if st.checkbox("📋 LIHAT DATA (50 Responden x 20 Soal)"):
    st.dataframe(df)
...
st.warning(f"🎯 Fokus Perbaikan: {prioritas}")
...
for soal, nilai in bottom5.items():
    st.write(f"- {soal} (rata-rata: {nilai:.2f}) → Perlu ditingkatkan")
